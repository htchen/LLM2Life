...
| **Lab 1** | 記帳小工具 | 33 分 |
| 🧠 洞見 1 | 你沒辦法把還沒想清楚的事外包出去 | 3 分 |
| 休息 | | 10 分 |
| **Lab 2** | GitHub Actions 每日摘要 | 34 分 |
| 🧠 洞見 2 | 值得自動化的門檻，剛剛降到你腳邊 | 3 分 |
| **Lab 3** | 習慣儀表板 + GitHub Pages | 33 分 |
| 🧠 洞見 3 | 越難收回的事，越要親眼看過再放行 | 3 分 |
| 成果分享 | | 5 分 |
| 🧠 **收束論證** | 今天真正發生的事 | 12 分 |
| 收尾 | | 4 分 |
...
每段洞見只講學員**十分鐘前剛親身經歷過的事**。非程式背景的聽眾沒有背景知識去評估
抽象主張，但他們對自己剛剛的感受有絕對的權威。

**把這些搬到開場當「理念說明」就毀了**——會變成聽不懂的空話。順序是設計的一部分。

論證的軸心是一句話：做軟體一直需要三件事——**知道要什麼**、**翻成機器聽得懂的話**、
**確認它真的對**。過去第二件貴到讓另外兩件顯得不重要，今天它幾乎免費了，
而剩下的第一件和第三件**從來就不是程式能力**。

收束論證裡有一段「誠實的那一半」（四個限制），**不能省**——
沒有它，整場就是推銷。
