# 講師統一付費：用 Console 額度供學員使用

課前通知信預設要求學員**自行訂閱 Pro 方案**。若改為**由主辦方統一付費**，
這份文件是完整做法。

結論先講：**可行，而且有一個為此設計的功能**——Console 組織的
**Claude Code workspace 支援「每位使用者」的月度花費上限**，這是唯一支援 per-user
spend limit 的 workspace。可為每位學員各設一個上限，依實際用量計費，課後一次回收。

---

## 免費帳號不包含 Claude Code

**「讓學員用免費帳號、選 Sonnet 模型」是行不通的**——而且原因不是模型，是產品。

官方定價頁把 **Claude Code 列為 Pro 以上專屬**，Free 那一欄沒有打勾。
免費帳號**確實可以用 Sonnet**（Free 方案能用 Fable、Opus、Sonnet），
但只能在 **claude.ai 對話**裡用，**不包含 Claude Code**。

換句話說：

> **限制來自「方案」，而非「模型」。**
> 換成 Sonnet、Haiku 或任何模型都不會讓免費帳號解鎖 Claude Code。

順帶一提，**要求學員自己買一個月 Pro 再退訂，比 Console 額度貴**：
Pro 約 US$17–20／人，30 人就是 $510–600；Console 額度按用量算，
同樣 30 人約 $450–750，而且用不完的部分留著下次開課。

### 免費版本

`claude.ai 對話（免費）＋ GitHub 網頁介面（免費）` 可以做出一個零安裝、零成本的版本：
學員在對話裡請 Claude 寫出程式碼，複製，貼到 GitHub 網頁的
**Add file → Create new file**，commit，再用 GitHub Pages 打開。

| 階段 | 免費版的操作差異 |
| --- | --- |
| 共同練習 | 取得完整程式碼後，手動建立與更新檔案 |
| 發想與規劃 | 操作方式相同，不產生程式碼 |
| 個人專題 | 每輪都要複製完整版本、存檔、測試並上傳 GitHub |
| GitHub Actions 備用案例 | Actions 仍在 GitHub 執行，可手動建立 workflow 檔案 |

開放式專題流程仍然適用：問題、目標、範圍、驗收與下一步都由學員判斷。

但操作方式會改變。原版投影片第 10–11 頁以一般聊天介面的文字回覆，
對比 Claude Code 可直接修改檔案；免費版則需要由學員自行複製、貼上與建立檔案。

還有兩個現實問題：

- **免費方案有用量上限**（Free 對 Fable / Opus 是週上限的 50%）。三小時密集使用，
  會有學員在課程中途被擋住。真的要走這條路，請學員在對話裡**指定用 Sonnet**以節省額度。
- **非程式背景的學員最常卡在「存成 .html 檔」**。用 GitHub 網頁建檔可以繞過這件事，
  但多了「先 commit 才看得到」的延遲。

**適合使用免費版的情況：** 場地是限制安裝軟體的電腦教室、沒有工具預算、
或線上開課且無法確認學員裝置。這時可改用另一套課程名稱：
「用 AI 和 GitHub 做出第一個教學小工具」，而非「用 Claude Code…」。

**這個版本已經完整寫好了**，見 [`../workshop/`](../workshop/)——
完整的 slides／workbook／site／instructor 自成一套，不是這份教材改幾個字。
隨堂網站：https://htchen.github.io/LLM2Life/handbook.html

---

## 三條路線比較

| | Console 額度（推薦） | Team 方案席位 | 學員自付 Pro（目前預設） |
| --- | --- | --- | --- |
| 誰付錢 | 主辦方（預付額度） | 主辦方（每席月費） | 學員自己 |
| 計費方式 | **按實際 token 用量** | 每席固定月費 | 學員的訂閱 |
| 每人花費上限 | ✅ **per-user 月度上限** | 不適用 | 不適用 |
| 桌面版可用 | ⚠️ **要先測**（見下） | ✅ | ✅ |
| 課後收回 | 把學員移出組織，金鑰立刻失效 | 取消席位（注意計費週期） | 學員自己取消 |
| 30 人三小時估算 | **約 US$450–750** | 30 席月費（通常貴得多） | $0（但會流失學員） |
| 課前準備 | 約 1 小時 | 約 30 分鐘 | 只要寄信 |

一次性的三小時工作坊可優先評估 Console 額度，因為它按用量計費，
不必為了一個下午買 30 個月費席位。

---

## ⚠️ 先做這個測試（10 分鐘，決定整場的路線）

**這一步不能跳過。** 官方文件明確寫著：

> `apiKeyHelper`、`ANTHROPIC_API_KEY`、`ANTHROPIC_AUTH_TOKEN` 適用於 CLI 及包裝 CLI 的介面……
> **Claude Desktop 和 cloud sessions 不會讀取這些環境變數，它們使用 OAuth。**

所以**發 API key 給學員，在桌面版是不會生效的**。

但 Console **帳號登入**（不是 API key）是 Claude Code 官方支援的登入方式之一，
而桌面版本來就走 OAuth。問題是：桌面版的文件只列出 Pro / Max / Team / Enterprise，
**沒有列 Console**——這一點文件沒有明說，我也無法從文件確認。

### 怎麼測

1. 於 Console 組織中邀請**一個測試帳號**（使用另一個信箱）
2. 給它 **Claude Code** 角色
3. 用那個帳號在**桌面版**點 **Code** 分頁，看能不能登入
4. 再用同一個帳號在**終端機**跑 `claude` → `/login` → 選 Console

| 測試結果 | 走哪條路線 |
| --- | --- |
| 桌面版能登入 | 使用現有教材，只改課前通知信 |
| 只有終端機能登入 | 教材要改成終端機路線（見下方「如果只有終端機能用」） |

**課前一週就測**，不要等到當天。

---

## 設定步驟（課前做，約 1 小時）

### 1. 建立 Console 組織並預付額度

1. 到 https://platform.claude.com 註冊/登入
2. **Billing** → 加入預付額度（金額見下方估算）

### 2. 邀請學員

**Settings → Members → Invite**，可以貼一整批信箱批次邀請。

角色一定要選 **Claude Code**：

> - **Claude Code** 角色：使用者只能建立 Claude Code 的 API 金鑰
> - **Developer** 角色：可以建立任何種類的金鑰

**選 Claude Code，不要選 Developer。** 這是最小權限——學員只能用 Claude Code，
不得挪用該額度執行其他用途。

### 3. 學員第一次登入後，設定每人上限 ⭐

**Claude Code workspace 是自動出現的**——組織裡第一個學員用 Console 帳號登入
Claude Code 時，Anthropic 會自動建立它，之後每個登入的人都會被加進去。

它的特別之處（官方文件原文）：

> - Claude Code 在登入時會在這個 workspace 為每位使用者鑄造一把個人 API 金鑰。
> - 如果金鑰的擁有者被移出 workspace 或組織，該金鑰會停止運作。
> - Claude Code 的用量有獨立的速率限制，管理員可以在 Settings > Workspaces 限制它佔組織額度的比例。
> - **它是唯一支援 per-user 月度花費上限的 workspace。**

到 **Settings → Workspaces → Claude Code → Spend limits**，設定**每位使用者的月度上限**。

> ⚠️ **這一步要等到有人登入過之後才做得到**，因為 workspace 是登入時才被建立的。
> 建議流程：課前先以測試帳號登入一次 → workspace 出現 → 設定上限 →
> 再邀請全班。這樣不會有學員在沒有上限的狀態下開始用。

### 4. 另外確認兩項設定

- **Rate limits 分頁**：限制 Claude Code 佔組織速率上限的比例
- 記下 **Usage & Cost** 頁面，課中可以即時看花費

### 5. 課後移除存取權

**Settings → Members** → 移除學員。他們的 Claude Code 金鑰**立刻失效**
（這是 Claude Code workspace 的特性，一般 workspace 的金鑰不會這樣）。

剩下的額度留在組織裡，下次開課還能用。

---

## 花費估算

以一位學員三小時、約 50 次對話、完成共同練習與一個個人專題來估：

| | Claude Sonnet 5 | Claude Opus 5 |
| --- | --- | --- |
| 定價（每百萬 token） | $3 輸入 / $15 輸出 | $5 輸入 / $25 輸出 |
| 每位學員估算 | **約 $4–6** | **約 $8–12** |
| **建議上限（含 2–3 倍餘裕）** | **$15 / 人** | **$25 / 人** |
| 30 人總預算 | **$450** | **$750** |

**估算怎麼來的：** 約 2.2M 輸入 token ＋ 約 120K 輸出 token。
Claude Code 大量使用 prompt caching，cache read 只算 0.1 倍價格，
所以輸入的實際成本遠低於帳面上的 token 數。

> ⚠️ **這份估算比第一版高，原因是快取存活時間。**
> 官方文件：「快取存活時間在**訂閱方案**上是**一小時**……在 **API key 或雲端供應商上，
> 預設是五分鐘**。」
>
> Console 路線在登入時是為每位使用者鑄造 API 金鑰，所以很可能吃到**五分鐘**那一邊。
> 課堂操作會因聽講、閱讀步驟與等待協助而出現較長停頓。
> 只要超過五分鐘沒送出訊息，下一則訊息就是 cache miss，
> **整個對話脈絡要用 1.25 倍的寫入價重跑一次**，而不是 0.1 倍的讀取價。
>
> 這正是我把估算從 $2–4 上調到 $4–6 的原因。
>
> **可以試著補救：** 設定環境變數 `ENABLE_PROMPT_CACHING_1H=1` 可以保住一小時的存活時間
> （文件是在「使用 usage credits 時」的脈絡提到它）。**這個變數對 Console 金鑰認證是否
> 生效，官方文件未明確說明**——請於試跑中實測，以 `/usage` 觀察 cache read 對 cache write
> 的比例有沒有改善。

**這堂課用 Sonnet 就夠了**——共同練習與個人專題都限制為小型、低風險的單檔工具，
不需要 Opus 的深度推理，而且省一半以上。可以在 workshop 開始時請學員選 Sonnet。

> 💰 **Sonnet 5 目前有introductory定價（$2/$10），到 2026-08-31 為止。**
> 若工作坊在該時點之後舉辦，請以上表標準價估算。

**三個會讓實際花費超出估算的情況：**

1. **有人遇到問題並反覆重試同一項操作** —— 可透過 per-user 上限控制費用
2. **有人未依範圍限制增加大量功能或外部資料** —— 會增加對話長度與重試次數
3. **課後持續使用** —— 上限以**月**為單位，學員課後繼續使用仍會計入主辦方費用。
   如果不希望這樣，課後就把人移除；如果希望鼓勵他們繼續，就留著並把上限設高一點
   （可依課後練習目的決定）

**先進行一次試算：** 課前安排一人完整走完共同練習與個人專題，
看 Usage & Cost 實際跳多少，再乘上人數。這比任何估算都準。

---

## ⚠️ 速率限制：30 人同時操作的風險

這項設定容易被忽略，且可能導致全班同時無法操作。

新開的 Console 組織速率上限（rate limits）通常在較低的等級，會隨著付費紀錄提升。
30 個學員在同一分鐘一起送出提示詞，可能達到**組織層級**的速率上限，
症狀是全班同時開始出現 429 錯誤。

**官方文件直接點名了這個情境：**

> **注意：** 若預期出現異常高的並行使用（**例如大型團體的即時教學課程**），
> 可能需為每位使用者配置更高的 TPM。

文件也給了每位使用者的建議值，可以直接拿去跟 Anthropic 要：

| 團隊人數 | 每人 TPM | 每人 RPM |
| --- | --- | --- |
| 5–20 人 | 100k–150k | 2.5–3.5 |
| **20–50 人** | **50k–75k** | **1.25–1.75** |
| 50–100 人 | 25k–35k | 0.62–0.87 |

**30 人的班級 → 約 30 × 60k =「1.8M TPM」**，即申請信中應載明的數字。
（這些上限是**組織層級**的，不是每人各自獨立，所以沒人用的時候個別學員可以暫時超用。）

**課前一定要做：**

1. 預付足夠的額度（額度將影響 tier 等級）
2. **開課前先去信 Anthropic support 或業務窗口**，說明
   「某月某日會有 30 位使用者同時使用 Claude Code 約三小時，需要約 1.8M TPM」。
   這封信要提早寄，不要課前一天才寄。
3. 課堂上把 Lab 開始時間錯開幾十秒（例如按座位分兩批放人動手），
   避免全班同時按 Enter

---

## 如果只有終端機能用（測試結果是桌面版不行）

教材要改。**這不是小改動**——現有設計刻意避開終端機，因為對非程式背景的學員
這會增加非程式背景學員的操作負擔。

要改的地方：

| 檔案 | 要改什麼 |
| --- | --- |
| `pre-workshop-email.md` | 刪除「自行訂閱 Pro」整段，改為「將收到一封 Console 邀請信，請接受」。安裝改成 CLI（`curl -fsSL https://claude.ai/install.sh \| bash`，Windows 用 PowerShell 版） |
| `lab-0-setup.md` | 步驟 1 改成：開終端機 → `claude` → `/login` → 選 **Console** |
| `slides/deck.md` | 「不會用到終端機」那張要拿掉（第 5 頁），Lab 0 的投影片要改 |
| `site/index.html` | 「開始之前」整段改寫 |
| `timeline.md` | **Lab 0 從 20 分鐘拉到 30 分鐘**，時間從別處挪 |

**還要加一段終端機操作說明**：怎麼開終端機、`cd` 是什麼、貼上為什麼不能用
Ctrl+V（macOS 用 Cmd+V）。非程式背景的學員在這裡需要更多協助。

**折衷方案（建議）：** 主要路線仍用桌面版 + 學員自己的 Pro 方案，
**Console 額度只給「沒有付費方案」的那 10–20% 學員當備案**，走終端機。
這樣多數人使用主要路線，少數人使用備用路線，且只需為備用路線準備額度。

---

## 另一條路：Claude for Education（值得問，但要提早）

Anthropic 有 **Claude for Education** 機構方案，**包含 Claude Code**。
合作學校的學生用 `.edu` 信箱登入 claude.ai 就會自動升級，**對學生免費**
（由學校與 Anthropic 談定）。

主辦方信箱為 `cs.nthu.edu.tw`，因此有兩件事值得處理：

1. 問學校的計算機中心/圖書館，清華有沒有既有的 Anthropic 機構協議
2. 沒有的話，透過 https://www.anthropic.com/contact-sales 詢問 Claude for Education

這條路的前置時間是數週到數月，不適用於近期工作坊。
它是「把這堂課變成常設課程」時該走的路，不是一次性活動的解法。

參考：
[Claude for Education（Stanford 的說明頁）](https://uit.stanford.edu/service/claude)、
[Anthropic × CodePath 合作案](https://www.anthropic.com/news/anthropic-codepath-partnership)

---

## 講師檢查表

課前兩週：

- [ ] 建 Console 組織、加預付額度
- [ ] **用測試帳號驗證桌面版能不能用 Console 登入**（決定路線）
- [ ] 寄信給 Anthropic 確認速率上限夠 30 人同時使用
- [ ] 問學校有沒有 Claude for Education 協議（長期方案）

課前一週：

- [ ] 讓測試帳號登入一次 Claude Code → Claude Code workspace 出現
- [ ] **先設好 per-user 月度花費上限**，再邀請全班
- [ ] 設 Claude Code 的 rate limit 佔比
- [ ] 自己完整跑一遍共同練習與個人專題，看實際花費，校正預算
- [ ] 批次邀請學員（角色選 **Claude Code**，不要選 Developer）
- [ ] 改好 `pre-workshop-email.md` 並寄出

當天：

- [ ] 開著 Usage & Cost 頁面，隨時看花費
- [ ] 錯開各 Lab 的開始時間，避免同時撞速率上限

課後：

- [ ] 決定要不要讓學員繼續用（月度上限會繼續計費）
- [ ] 不繼續的話：Settings → Members 移除學員，金鑰立刻失效
